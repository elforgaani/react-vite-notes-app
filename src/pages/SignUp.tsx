          ...
          <motion.div variants={container} initial='hidden' animate="visible" className='rounded-xl shadow-xl grid grid-cols-1  md:grid-cols-2 w-full  p-14 md:p-12 bg-base-200 gap-x-4' >
            <div className="login-form flex flex-col justify-center items-center bg-base-200 gap-5">
              ...
              <motion.label variants={item} className="input focus:outline-neutral focus-within:outline-neutral input-bordered flex items-center gap-2 w-full md:w-[60%] ">
                ...
                <input type="text" className="grow" placeholder="name" name='name' id='name' onChange={formik.handleChange} onBlur={formik.handleBlur} />
              </motion.label>
              ...
              <motion.label variants={item} className="input focus:outline-neutral focus-within:outline-neutral input-bordered flex items-center gap-2 w-full md:w-[60%] ">
                ...
                <input type="text" className="grow" placeholder="Email" name='email' onChange={formik.handleChange} onBlur={formik.handleBlur} id='email' />
              </motion.label>
              ...
              <motion.label variants={item} className="input focus:outline-neutral focus-within:outline-neutral  input-bordered flex items-center gap-2 w-full md:w-[60%]">
                ...
                <input type="password" className="grow" name='password' placeholder="password" onChange={formik.handleChange} onBlur={formik.handleBlur} id='password' />
              </motion.label>
              {formik.touched.password && formik.errors.password && <WarningAlert message={formik.errors.password} />
              }
              <motion.div variants={item} className='flex justify-end w-full md:w-[60%]'>
                <Link to='/forget-password' className='text-neutral'>Forget Password?</Link>
              </motion.div>
              <motion.button disabled={(!formik.isValid || isPending) ?? false} type='submit' variants={item} className="btn btn-primary w-full md:w-[30%] text-white  text-sm">{isPending ? 'Loading ..' : 'Sign Up'}</motion.button>
              {/* Todo: Need to be edited */}
              <motion.div variants={item} className='flex justify-start items-center flex-wrap w-full md:w-[60%]'>
                <p className='text-primary-content'>Already have an account?</p>
                <Link to={paths.signIn} className='text-neutral'>Sign In</Link>
              </motion.div>
            </div>
            <div>
